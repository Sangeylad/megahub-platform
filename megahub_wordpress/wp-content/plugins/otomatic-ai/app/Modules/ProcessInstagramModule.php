...
class ProcessInstagramModule implements ModuleContract
{
    /**
     * The publication
     *
    ...
    public function handle(): void
    {
        ...
        $this->publication->addLog("Instagram module started.");

        // determine length
        $length = max(1, intval(Arr::get($this->publication->project->modules, "instagram.length", 1)));
        ...
        try {
            ...
            // call the api
            $api = new Client;
            $result = $api->instagram($search, $this->publication->project->language->key);
            ...
            if (!empty($posts)) {
                ...
                $posts = array_map(function ($post) {
                    return new Instagram($post["link"]);
                }, $posts);
                ...
                switch (Arr::get($this->publication->project->modules, 'instagram.position')) {
                    case "top":
                ...
                $this->publication->addLog("Instagram module completed successfully.", "success");
            } else {
                $this->publication->addLog("Instagram module failed. No posts found.", "warning");
            }
        } catch (Exception $e) {
            $this->publication->addLog("Instagram module failed. " . $e->getMessage(), "warning");
        }
    }

    /**
     * Make the Instagram search terms for the provided search
     *
     * @param string $search
     * @return string
     * @throws Exception
    ...
    {
        return Arr::get($publication->project->modules, "instagram.enabled", false);
    }
    ...
    static public function isRunnable(Publication $publication): bool
    {
        ...
        if (intval(Arr::get($publication->project->modules, "instagram.length", 1)) <= 0) {
            return false;
        }

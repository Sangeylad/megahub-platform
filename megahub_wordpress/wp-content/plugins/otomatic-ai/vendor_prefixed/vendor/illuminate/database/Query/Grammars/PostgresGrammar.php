...
class PostgresGrammar extends Grammar
{
    ...
    public function whereFullText(Builder $query, $where)
    {
        ...
        $columns = \OtomaticAi\Vendors\collect($where['columns'])->map(function ($column) use($language) {
    ...
    protected function compileUpdateColumns(Builder $query, array $values)
    {
        return \OtomaticAi\Vendors\collect($values)->map(function ($value, $key) {
            $column = \OtomaticAi\Vendors\last(\explode('.', $key));
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $columns = \OtomaticAi\Vendors\collect($update)->map(function ($value, $key) {
    ...
    public function compileUpdateFrom(Builder $query, $values)
    {
        ...
        if (isset($query->joins)) {
            ...
            $froms = \OtomaticAi\Vendors\collect($query->joins)->map(function ($join) {
    ...
    {
        $values = \OtomaticAi\Vendors\collect($values)->map(function ($value, $column) {
    ...
    protected function compileUpdateWithJoinsOrLimit(Builder $query, array $values)
    {
        $table = $this->wrapTable($query->from);
        $columns = $this->compileUpdateColumns($query, $values);
        $alias = \OtomaticAi\Vendors\last(\preg_split('/\\s+as\\s+/i', $query->from));

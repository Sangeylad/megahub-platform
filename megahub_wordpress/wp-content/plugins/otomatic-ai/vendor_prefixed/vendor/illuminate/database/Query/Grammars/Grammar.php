...
namespace OtomaticAi\Vendors\Illuminate\Database\Query\Grammars;

use OtomaticAi\Vendors\Illuminate\Database\Grammar as BaseGrammar;
use OtomaticAi\Vendors\Illuminate\Database\Query\Builder;
...
class Grammar extends BaseGrammar
{
    ...
    {
        return \OtomaticAi\Vendors\collect($joins)->map(function ($join) use($query) {
    ...
    {
        return \OtomaticAi\Vendors\collect($query->wheres)->map(function ($where) use($query) {
    ...
    protected function compileHavingBetween($having)
    {
        ...
        $column = $this->wrap($having['column']);
        $min = $this->parameter(\OtomaticAi\Vendors\head($having['values']));
        $max = $this->parameter(\OtomaticAi\Vendors\last($having['values']));
    ...
    public function compileInsert(Builder $query, array $values)
    {
        ...
        $parameters = \OtomaticAi\Vendors\collect($values)->map(function ($record) {
    ...
    {
        return \OtomaticAi\Vendors\collect($values)->map(function ($value, $key) {
    ...
    {
        $alias = \OtomaticAi\Vendors\last(\explode(' as ', $table));
        $joins = $this->compileJoins($query, $query->joins);

...
use Closure;
use OtomaticAi\Vendors\Illuminate\Support\Reflector;
...
trait ReflectsClosures
{
    ...
    protected function firstClosureParameterTypes(Closure $closure)
    {
        ...
        $types = \OtomaticAi\Vendors\collect($reflection->getParameters())->mapWithKeys(function ($parameter) {
    ...
    {
        $reflection = new ReflectionFunction($closure);
        return \OtomaticAi\Vendors\collect($reflection->getParameters())->mapWithKeys(function ($parameter) {
            if ($parameter->isVariadic()) {
                return [$parameter->getName() => null];
            }

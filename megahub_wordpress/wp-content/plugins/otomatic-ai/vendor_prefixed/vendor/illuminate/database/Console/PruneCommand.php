...
namespace OtomaticAi\Vendors\Illuminate\Database\Console;

use OtomaticAi\Vendors\Illuminate\Console\Command;
use OtomaticAi\Vendors\Illuminate\Contracts\Events\Dispatcher;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\MassPrunable;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Prunable;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\SoftDeletes;
use OtomaticAi\Vendors\Illuminate\Database\Events\ModelsPruned;
...
class PruneCommand extends Command
{
    ...
    protected function models()
    {
        if (!empty($models = $this->option('model'))) {
            return \OtomaticAi\Vendors\collect($models)->filter(function ($model) {
        ...
        return \OtomaticAi\Vendors\collect((new Finder())->in($this->getDefaultPath())->files()->name('*.php'))->map(function ($model) {
    ...
    {
        $uses = \OtomaticAi\Vendors\class_uses_recursive($model);
    ...
    {
        $instance = new $model();
        $count = $instance->prunable()->when(\in_array(SoftDeletes::class, \OtomaticAi\Vendors\class_uses_recursive(\get_class($instance))), function ($query) {
            $query->withTrashed();
        })->count();
        if ($count === 0) {
            $this->info("No prunable [{$model}] records found.");

...
use OtomaticAi\Vendors\andreskrey\Readability\Nodes\DOM\DOMDocument;
use OtomaticAi\Vendors\andreskrey\Readability\Nodes\DOM\DOMElement;
use OtomaticAi\Vendors\andreskrey\Readability\Nodes\DOM\DOMNode;
use OtomaticAi\Vendors\andreskrey\Readability\Nodes\DOM\DOMText;
use OtomaticAi\Vendors\andreskrey\Readability\Nodes\NodeUtility;
...
class Readability
{
     ...
     * @var array
     */
    private $defaultTagsToScore = ['section', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'td', 'pre'];
    /**
     * @var array
     */
    private $alterToDIVExceptions = ['div', 'article', 'section', 'p'];
    /**
    ...
    public function parse($html)
    {
        ...
        if (!$this->getExcerpt()) {
            ...
            if ($paragraphs->length > 0) {
                $this->setExcerpt(\trim($paragraphs->item(0)->textContent));
            }
        }
    ...
    private function getMetadata()
    {
         ...
         */
        $key = \current(\array_intersect(['dc:title', 'dcterm:title', 'og:title', 'weibo:article:title', 'weibo:webpage:title', 'title', 'twitter:title'], \array_keys($values)));
        $this->setTitle(isset($values[$key]) ? \trim($values[$key]) : null);
        if (!$this->getTitle()) {
            $this->setTitle($this->getArticleTitle());
        }
        // get author
        $key = \current(\array_intersect(['dc:creator', 'dcterm:creator', 'author'], \array_keys($values)));
        $this->setAuthor(isset($values[$key]) ? $values[$key] : null);
        // get description
        $key = \current(\array_intersect(['dc:description', 'dcterm:description', 'og:description', 'weibo:article:description', 'weibo:webpage:description', 'description', 'twitter:description'], \array_keys($values)));
        $this->setExcerpt(isset($values[$key]) ? $values[$key] : null);
        // get main image
        $key = \current(\array_intersect(['image', 'og:image', 'twitter:image'], \array_keys($values)));
        $this->setImage(isset($values[$key]) ? $values[$key] : null);
        $key = \current(\array_intersect(['og:site_name'], \array_keys($values)));
    ...
    private function getArticleTitle()
    {
        ...
        $curTitle = $originalTitle = \trim($originalTitle);
        ...
        if (\preg_match('/ [\\|\\-\\\\\\/>»] /i', $curTitle)) {
            ...
            for ($i = 1; $i <= 2; $i++) {
                foreach ($this->dom->getElementsByTagName('h' . $i) as $hTag) {
                    ...
                    if (\trim($hTag->nodeValue) === \trim($curTitle)) {
                        $match = \true;
                    }
                }
            }
        ...
        if ($curTitleWordCount <= 4 && (!$titleHadHierarchicalSeparators || $curTitleWordCount !== $originalTitleWordCount)) {
            $curTitle = $originalTitle;
    ...
    private function getNodes($node)
    {
        ...
        while ($node) {
            ...
            if ($stripUnlikelyCandidates) {
                if (\preg_match(NodeUtility::$regexps['unlikelyCandidates'], $matchString) && !\preg_match(NodeUtility::$regexps['okMaybeItsACandidate'], $matchString) && $node->nodeName !== 'body' && $node->nodeName !== 'a') {
            ...
            if (($node->nodeName === 'div' || $node->nodeName === 'section' || $node->nodeName === 'header' || $node->nodeName === 'h1' || $node->nodeName === 'h2' || $node->nodeName === 'h3' || $node->nodeName === 'h4' || $node->nodeName === 'h5' || $node->nodeName === 'h6' || $node->nodeName === 'p') && $node->isElementWithoutContent()) {
    ...
    private function checkByline($node, $matchString)
    {
        ...
        if ($rel === 'author' || \preg_match(NodeUtility::$regexps['byline'], $matchString) && $this->isValidByline($node->getTextContent())) {
            ...
            $this->setAuthor(\trim($node->getTextContent()));
            return \true;
        }
        return \false;
    }
    ...
    private function rateNodes($nodes)
    {
        ...
        foreach ($nodes as $node) {
            ...
            if (\mb_strlen($node->getTextContent(\true)) < 25) {
                continue;
            }
            ...
            $contentScore += \count(\explode(',', $node->getTextContent(\true)));
            // For every 100 characters in this paragraph, add another point. Up to 3 points.
            $contentScore += \min(\floor(\mb_strlen($node->getTextContent(\true)) / 100), 3);
        ...
        if ($topCandidate === null || $topCandidate->nodeName === 'body') {
            ...
            for ($i = 0; $i < $kids->length; $i++) {
                $import = $topCandidate->importNode($kids->item($i), \true);
        ...
        for ($i = 0; $i < $siblings->length; $i++) {
            ...
            if ($sibling === $topCandidate) {
                ...
                if ($sibling->contentScore + $contentBonus >= $siblingScoreThreshold) {
                    ...
                    $nodeContent = $sibling->getTextContent(\true);
            ...
            if ($append) {
                ...
                $import = $articleContent->importNode($sibling, \true);
    ...
    public function _markDataTables(DOMDocument $article)
    {
        ...
        foreach ($tables as $table) {
            ...
            if ($summary) {
                $table->setReadabilityDataTable(\true);
                continue;
            }
    ...
    {
        $endOfSearchMarkerNode = NodeUtility::getNextNode($node, \true);
    ...
    public function _cleanConditionally(DOMDocument $article, $tag)
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            if (\substr_count($node->getTextContent(), ',') < 10) {
                ...
                $contentLength = \mb_strlen($node->getTextContent(\true));
                $haveToRemove = $img > 1 && $p / $img < 0.5 && !$node->hasAncestorTag('figure') || !$isList && $li > $p || $input > \floor($p / 3) || !$isList && $contentLength < 25 && ($img === 0 || $img > 2) && !$node->hasAncestorTag('figure') || !$isList && $weight < 25 && $linkDensity > 0.2 || $weight >= 25 && $linkDensity > 0.5 || ($embedCount === 1 && $contentLength < 75 || $embedCount > 1);
                if ($haveToRemove) {
    ...
    public function postProcessContent(DOMDocument $article)
    {
        ...
        if ($this->configuration->getFixRelativeURLs()) {
            ...
            foreach ($article->getElementsByTagName('img') as $img) {
                 ...
                 */
                $url = [$img->getAttribute('src'), $img->getAttribute('data-src'), $img->getAttribute('data-original'), $img->getAttribute('data-orig'), $img->getAttribute('data-url')];

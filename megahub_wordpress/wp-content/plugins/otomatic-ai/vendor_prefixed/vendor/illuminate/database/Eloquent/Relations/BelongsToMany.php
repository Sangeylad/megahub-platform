...
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\ModelNotFoundException;
...
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Relations\Concerns\InteractsWithPivotTable;
...
class BelongsToMany extends Relation
{
    ...
    protected function resolveTableName($table)
    {
        ...
        if (\in_array(AsPivot::class, \OtomaticAi\Vendors\class_uses_recursive($model))) {
            $this->using($table);
        }
    ...
    protected function aliasedPivotColumns()
    {
        ...
        return \OtomaticAi\Vendors\collect(\array_merge($defaults, $this->pivotColumns))->map(function ($column) {
    ...
    public function paginate($perPage = null, $columns = ['*'], $pageName = 'page', $page = null)
    {
        ...
        return \OtomaticAi\Vendors\tap($this->query->paginate($perPage, $columns, $pageName, $page), function ($paginator) {
    ...
    {
        $this->query->addSelect($this->shouldSelect($columns));
        return \OtomaticAi\Vendors\tap($this->query->simplePaginate($perPage, $columns, $pageName, $page), function ($paginator) {
            $this->hydratePivotRelation($paginator->items());
        });
    }
    /**
    ...
    {
        $this->query->addSelect($this->shouldSelect($columns));
        return \OtomaticAi\Vendors\tap($this->query->cursorPaginate($perPage, $columns, $cursorName, $cursor), function ($paginator) {
            $this->hydratePivotRelation($paginator->items());
        });
    }
    /**
    ...
    {
        return Str::camel(Str::pluralStudly(\OtomaticAi\Vendors\class_basename($this->getParent())));
    }
    /**

...
use OtomaticAi\Vendors\Illuminate\Contracts\Bus\QueueingDispatcher;
use OtomaticAi\Vendors\Illuminate\Support\Arr;
use OtomaticAi\Vendors\Illuminate\Support\Collection;
use OtomaticAi\Vendors\Illuminate\Support\Traits\ReflectsClosures;
...
class BusFake implements QueueingDispatcher
{
    ...
    public function assertChained(array $expectedChain)
    {
        ...
        PHPUnit::assertTrue(\OtomaticAi\Vendors\collect($expectedChain)->isNotEmpty(), 'The expected chain can not be empty.');
    ...
    {
        return \OtomaticAi\Vendors\tap(clone $job, function ($job) {
    ...
    {
        $chain = \OtomaticAi\Vendors\collect($expectedChain)->map(function ($job) {
    ...
    protected function assertDispatchedWithChainOfClasses($command, $expectedChain, $callback)
    {
        $matching = $this->dispatched($command, $callback)->map->chained->map(function ($chain) {
            return \OtomaticAi\Vendors\collect($chain)->map(function ($job) {
    ...
    {
        return !\OtomaticAi\Vendors\collect($chain)->contains(function ($job) {
    ...
    public function dispatched($command, $callback = null)
    {
        ...
        return \OtomaticAi\Vendors\collect($this->commands[$command])->filter(function ($command) use($callback) {
    ...
    public function dispatchedSync(string $command, $callback = null)
    {
        ...
        return \OtomaticAi\Vendors\collect($this->commandsSync[$command])->filter(function ($command) use($callback) {
            return $callback($command);
        });
    }
    /**
    ...
    public function dispatchedAfterResponse(string $command, $callback = null)
    {
        ...
        return \OtomaticAi\Vendors\collect($this->commandsAfterResponse[$command])->filter(function ($command) use($callback) {
            return $callback($command);
        });
    }
    /**
    ...
    public function batched(callable $callback)
    {
        ...
        return \OtomaticAi\Vendors\collect($this->batches)->filter(function ($batch) use($callback) {
    ...
    protected function shouldFakeJob($command)
    {
        ...
        return \OtomaticAi\Vendors\collect($this->jobsToFake)->filter(function ($job) use($command) {

...
class IpUtils
{
    ...
    {
        if (null === $requestIp) {
            \OtomaticAi\Vendors\trigger_deprecation('symfony/http-foundation', '5.4', 'Passing null as $requestIp to "%s()" is deprecated, pass an empty string instead.', __METHOD__);
            return \false;
        }
        if (!\is_array($ips)) {
            $ips = [$ips];

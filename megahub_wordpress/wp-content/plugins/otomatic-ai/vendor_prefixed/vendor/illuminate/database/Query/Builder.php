...
namespace OtomaticAi\Vendors\Illuminate\Database\Query;

use BackedEnum;
use Closure;
use DateTimeInterface;
use OtomaticAi\Vendors\Illuminate\Contracts\Support\Arrayable;
use OtomaticAi\Vendors\Illuminate\Database\Concerns\BuildsQueries;
use OtomaticAi\Vendors\Illuminate\Database\Concerns\ExplainsQueries;
use OtomaticAi\Vendors\Illuminate\Database\ConnectionInterface;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Builder as EloquentBuilder;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Relations\Relation;
use OtomaticAi\Vendors\Illuminate\Database\Query\Grammars\Grammar;
use OtomaticAi\Vendors\Illuminate\Database\Query\Processors\Processor;
use OtomaticAi\Vendors\Illuminate\Pagination\Paginator;
use OtomaticAi\Vendors\Illuminate\Support\Arr;
use OtomaticAi\Vendors\Illuminate\Support\Collection;
use OtomaticAi\Vendors\Illuminate\Support\LazyCollection;
...
class Builder
{
    ...
    {
        return \OtomaticAi\Vendors\collect($this->onceWithColumns(Arr::wrap($columns), function () {
    ...
    public function paginate($perPage = 15, $columns = ['*'], $pageName = 'page', $page = null)
    {
        ...
        $results = $total ? $this->forPage($page, $perPage)->get($columns) : \OtomaticAi\Vendors\collect();
    ...
    {
        $this->enforceOrderBy();
        return \OtomaticAi\Vendors\collect($this->orders ?? $this->unionOrders ?? [])->filter(function ($order) {
    ...
    public function pluck($column, $key = null)
    {
        ...
        if (empty($queryResult)) {
            return \OtomaticAi\Vendors\collect();
        }
    ...
    protected function stripTableForPluck($column)
    {
        ...
        return \OtomaticAi\Vendors\last(\preg_split('~' . $separator . '~i', $column));
    }
    /**
    ...
    protected function pluckFromObjectColumn($queryResult, $column, $key)
    {
        ...
        return \OtomaticAi\Vendors\collect($results);
    }
    /**
    ...
    public function upsert(array $values, $uniqueBy, $update = null)
    {
        ...
        $this->applyBeforeQueryCallbacks();
        $bindings = $this->cleanBindings(\array_merge(Arr::flatten($values, 1), \OtomaticAi\Vendors\collect($update)->reject(function ($value, $key) {
            return \is_int($key);
        })->all()));
    ...
    {
        return \OtomaticAi\Vendors\collect($bindings)->reject(function ($binding) {
    ...
    {
        return \is_array($value) ? \OtomaticAi\Vendors\head(Arr::flatten($value)) : $value;
    }
    /**
    ...
    {
        return \OtomaticAi\Vendors\tap($this->clone(), function ($clone) use($properties) {
    ...
    {
        return \OtomaticAi\Vendors\tap($this->clone(), function ($clone) use($except) {

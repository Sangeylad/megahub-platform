...
use ArrayAccess;
use OtomaticAi\Vendors\Illuminate\Contracts\Broadcasting\HasBroadcastChannel;
use OtomaticAi\Vendors\Illuminate\Contracts\Queue\QueueableCollection;
use OtomaticAi\Vendors\Illuminate\Contracts\Queue\QueueableEntity;
use OtomaticAi\Vendors\Illuminate\Contracts\Routing\UrlRoutable;
use OtomaticAi\Vendors\Illuminate\Contracts\Support\Arrayable;
use OtomaticAi\Vendors\Illuminate\Contracts\Support\CanBeEscapedWhenCastToString;
use OtomaticAi\Vendors\Illuminate\Contracts\Support\Jsonable;
use OtomaticAi\Vendors\Illuminate\Database\ConnectionResolverInterface as Resolver;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Collection as EloquentCollection;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Relations\BelongsToMany;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Relations\Concerns\AsPivot;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Relations\HasManyThrough;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Relations\Pivot;
use OtomaticAi\Vendors\Illuminate\Support\Arr;
use OtomaticAi\Vendors\Illuminate\Support\Collection as BaseCollection;
use OtomaticAi\Vendors\Illuminate\Support\Str;
use OtomaticAi\Vendors\Illuminate\Support\Traits\ForwardsCalls;
...
abstract class Model implements Arrayable, ArrayAccess, CanBeEscapedWhenCastToString, HasBroadcastChannel, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
    ...
    protected static function bootTraits()
    {
        ...
        foreach (\OtomaticAi\Vendors\class_uses_recursive($class) as $trait) {
            $method = 'boot' . \OtomaticAi\Vendors\class_basename($trait);
            if (\method_exists($class, $method) && !\in_array($method, $booted)) {
                \forward_static_call([$class, $method]);
                $booted[] = $method;
            }
            if (\method_exists($class, $method = 'initialize' . \OtomaticAi\Vendors\class_basename($trait))) {
    ...
    {
        return \OtomaticAi\Vendors\collect($columns)->map(function ($column) {
    ...
    protected function incrementOrDecrement($column, $amount, $extra, $method)
    {
        ...
        return \OtomaticAi\Vendors\tap($this->setKeysForSaveQuery($query)->{$method}($column, $amount, $extra), function () use($column) {
            $this->syncChanges();
    ...
    public function refresh()
    {
        ...
        $this->load(\OtomaticAi\Vendors\collect($this->relations)->reject(function ($relation) {
            return $relation instanceof Pivot || \is_object($relation) && \in_array(AsPivot::class, \OtomaticAi\Vendors\class_uses_recursive($relation), \true);
        })->keys()->all());
        $this->syncOriginal();
        return $this;
    }
    ...
    public function replicate(array $except = null)
    {
        ...
        return \OtomaticAi\Vendors\tap(new static(), function ($instance) use($attributes) {
    ...
    {
        return $this->table ?? Str::snake(Str::pluralStudly(\OtomaticAi\Vendors\class_basename($this)));
    }
    /**
    ...
    {
        return Str::snake(\OtomaticAi\Vendors\class_basename($this)) . '_' . $this->getKeyName();
    }
    /**
    ...
    {
        return \str_replace('\\', '.', \get_class($this)) . '.{' . Str::camel(\OtomaticAi\Vendors\class_basename($this)) . '}';
    }
    /**
    ...
    {
        return $this->escapeWhenCastingToString ? \OtomaticAi\Vendors\e($this->toJson()) : $this->toJson();
    }
    /**

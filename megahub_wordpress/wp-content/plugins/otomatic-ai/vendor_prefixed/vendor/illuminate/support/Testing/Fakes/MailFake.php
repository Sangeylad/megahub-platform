...
use Closure;
use OtomaticAi\Vendors\Illuminate\Contracts\Mail\Factory;
use OtomaticAi\Vendors\Illuminate\Contracts\Mail\Mailable;
use OtomaticAi\Vendors\Illuminate\Contracts\Mail\Mailer;
use OtomaticAi\Vendors\Illuminate\Contracts\Mail\MailQueue;
use OtomaticAi\Vendors\Illuminate\Contracts\Queue\ShouldQueue;
...
class MailFake implements Factory, Mailer, MailQueue
{
    ...
    {
        $mailableNames = \OtomaticAi\Vendors\collect($this->mailables)->map(function ($mailable) {
    ...
    {
        $mailableNames = \OtomaticAi\Vendors\collect($this->queuedMailables)->map(function ($mailable) {
            return \get_class($mailable);
        })->join(', ');
    ...
    {
        return \OtomaticAi\Vendors\collect($this->mailables)->filter(function ($mailable) use($type) {
    ...
    {
        return \OtomaticAi\Vendors\collect($this->queuedMailables)->filter(function ($mailable) use($type) {
            return $mailable instanceof $type;
        });
    }
    /**

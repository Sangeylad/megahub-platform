...
use OtomaticAi\Vendors\Illuminate\Database\Query\Builder as QueryBuilder;
use OtomaticAi\Vendors\Illuminate\Database\RecordsNotFoundException;
...
class Builder
{
    ...
    public function firstOrCreate(array $attributes = [], array $values = [])
    {
        ...
        return \OtomaticAi\Vendors\tap($this->newModelInstance(\array_merge($attributes, $values)), function ($instance) {
            $instance->save();
        });
    }
    /**
    ...
    protected function ensureOrderForCursorPagination($shouldReverse = \false)
    {
        ...
        if ($shouldReverse) {
            $this->query->orders = \OtomaticAi\Vendors\collect($this->query->orders)->map(function ($order) {
                $order['direction'] = $order['direction'] === 'asc' ? 'desc' : 'asc';
                return $order;
            })->toArray();
        }
        if ($this->query->unionOrders) {
            return \OtomaticAi\Vendors\collect($this->query->unionOrders);
        }
        return \OtomaticAi\Vendors\collect($this->query->orders);
    }
    /**
    ...
    {
        return \OtomaticAi\Vendors\tap($this->newModelInstance($attributes), function ($instance) {
            $instance->save();
        });
    }
    /**
    ...
    {
        $whereBooleans = \OtomaticAi\Vendors\collect($whereSlice)->pluck('boolean');

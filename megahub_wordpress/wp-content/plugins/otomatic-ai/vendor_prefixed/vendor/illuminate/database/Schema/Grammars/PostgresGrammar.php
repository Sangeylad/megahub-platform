...
class PostgresGrammar extends Grammar
{
    ...
    {
        return \OtomaticAi\Vendors\collect($blueprint->autoIncrementingStartingValues())->map(function ($value, $column) use($blueprint) {
    ...
    protected function generatableColumn($type, Fluent $column)
    {
        ...
        if ($column->autoIncrement && \is_null($column->generatedAs)) {
            return \OtomaticAi\Vendors\with(['integer' => 'serial', 'bigint' => 'bigserial', 'smallint' => 'smallserial'])[$type];
        }
        $options = '';

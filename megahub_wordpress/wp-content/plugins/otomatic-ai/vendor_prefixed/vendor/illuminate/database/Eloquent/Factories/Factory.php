...
use Closure;
use OtomaticAi\Vendors\Faker\Generator;
use OtomaticAi\Vendors\Illuminate\Container\Container;
use OtomaticAi\Vendors\Illuminate\Contracts\Foundation\Application;
...
abstract class Factory
{
    ...
    {
        return new EloquentCollection(\OtomaticAi\Vendors\collect($records)->map(function ($record) {
    ...
    public function create($attributes = [], ?Model $parent = null)
    {
        ...
        if ($results instanceof Model) {
            $this->store(\OtomaticAi\Vendors\collect([$results]));
            $this->callAfterCreating(\OtomaticAi\Vendors\collect([$results]), $parent);
        } else {
            $this->store($results);
    ...
    public function make($attributes = [], ?Model $parent = null)
    {
        ...
        if ($this->count === null) {
            return \OtomaticAi\Vendors\tap($this->makeInstance($parent), function ($instance) {
                $this->callAfterMaking(\OtomaticAi\Vendors\collect([$instance]));
            });
        }
        if ($this->count < 1) {
    ...
    protected function makeInstance(?Model $parent)
    {
        return Model::unguarded(function () use($parent) {
            return \OtomaticAi\Vendors\tap($this->newModel($this->getExpandedAttributes($parent)), function ($instance) {
                if (isset($this->connection)) {
    ...
    {
        return \OtomaticAi\Vendors\collect($definition)->map(function ($attribute, $key) use(&$definition) {
    ...
    {
        $guess = Str::camel(Str::plural(\OtomaticAi\Vendors\class_basename($related)));
    ...
    {
        return $this->newInstance(['has' => $this->has->concat([new BelongsToManyRelationship($factory, $pivot, $relationship ?: Str::camel(Str::plural(\OtomaticAi\Vendors\class_basename($factory instanceof Factory ? $factory->modelName() : Collection::wrap($factory)->first()))))])]);
    }
    /**
    ...
    {
        return $this->newInstance(['for' => $this->for->concat([new BelongsToRelationship($factory, $relationship ?: Str::camel(\OtomaticAi\Vendors\class_basename($factory instanceof Factory ? $factory->modelName() : $factory)))])]);
    }
    /**
    ...
    public function modelName()
    {
        $resolver = static::$modelNameResolver ?: function (self $factory) {
            ...
            $factoryBasename = Str::replaceLast('Factory', '', \OtomaticAi\Vendors\class_basename($factory));

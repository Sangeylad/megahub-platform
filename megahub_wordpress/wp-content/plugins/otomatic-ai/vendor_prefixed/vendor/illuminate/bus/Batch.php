...
use Closure;
use OtomaticAi\Vendors\Illuminate\Contracts\Queue\Factory as QueueFactory;
use OtomaticAi\Vendors\Illuminate\Contracts\Support\Arrayable;
use OtomaticAi\Vendors\Illuminate\Queue\CallQueuedClosure;
...
class Batch implements Arrayable, JsonSerializable
{
    ...
    public function add($jobs)
    {
        ...
        $jobs = Collection::wrap($jobs)->map(function ($job) use(&$count) {
            ...
            if (\is_array($job)) {
                $count += \count($job);
                return \OtomaticAi\Vendors\with($this->prepareBatchedChain($job), function ($chain) {
    ...
    public function recordSuccessfulJob(string $jobId)
    {
        ...
        if ($counts->pendingJobs === 0 && $this->hasThenCallbacks()) {
            $batch = $this->fresh();
            \OtomaticAi\Vendors\collect($this->options['then'])->each(function ($handler) use($batch) {
                $this->invokeHandlerCallback($handler, $batch);
            });
        }
        if ($counts->allJobsHaveRanExactlyOnce() && $this->hasFinallyCallbacks()) {
            $batch = $this->fresh();
            \OtomaticAi\Vendors\collect($this->options['finally'])->each(function ($handler) use($batch) {
                $this->invokeHandlerCallback($handler, $batch);
            });
        }
    }
    ...
    public function recordFailedJob(string $jobId, $e)
    {
        ...
        if ($counts->failedJobs === 1 && $this->hasCatchCallbacks()) {
            $batch = $this->fresh();
            \OtomaticAi\Vendors\collect($this->options['catch'])->each(function ($handler) use($batch, $e) {
                $this->invokeHandlerCallback($handler, $batch, $e);
            });
        }
        if ($counts->allJobsHaveRanExactlyOnce() && $this->hasFinallyCallbacks()) {
            $batch = $this->fresh();
            \OtomaticAi\Vendors\collect($this->options['finally'])->each(function ($handler) use($batch, $e) {
                $this->invokeHandlerCallback($handler, $batch, $e);
            });
        }
    }
    ...
    protected function invokeHandlerCallback($handler, Batch $batch, Throwable $e = null)
    {
        try {
            ...
            if (\function_exists('OtomaticAi\\Vendors\\report')) {
                report($e);
            }
        }
    }

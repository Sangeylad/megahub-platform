...
use OtomaticAi\Vendors\Illuminate\Contracts\Broadcasting\Factory as BroadcastFactory;
use OtomaticAi\Vendors\Illuminate\Contracts\Broadcasting\ShouldBroadcast;
use OtomaticAi\Vendors\Illuminate\Contracts\Container\Container as ContainerContract;
use OtomaticAi\Vendors\Illuminate\Contracts\Events\Dispatcher as DispatcherContract;
use OtomaticAi\Vendors\Illuminate\Contracts\Queue\ShouldBeEncrypted;
...
class Dispatcher implements DispatcherContract
{
    ...
    public function listen($events, $listener = null)
    {
        if ($events instanceof Closure) {
            return \OtomaticAi\Vendors\collect($this->firstClosureParameterTypes($events))->each(function ($event) use($events) {
                $this->listen($event, $events);
            });
        } elseif ($events instanceof QueuedClosure) {
            return \OtomaticAi\Vendors\collect($this->firstClosureParameterTypes($events->closure))->each(function ($event) use($events) {
    ...
    {
        return \OtomaticAi\Vendors\tap($job, function ($job) use($listener) {

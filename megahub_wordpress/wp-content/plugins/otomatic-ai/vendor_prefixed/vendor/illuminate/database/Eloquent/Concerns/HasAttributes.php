...
namespace OtomaticAi\Vendors\Illuminate\Database\Eloquent\Concerns;
...
use OtomaticAi\Vendors\Illuminate\Contracts\Database\Eloquent\CastsInboundAttributes;
use OtomaticAi\Vendors\Illuminate\Contracts\Support\Arrayable;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Casts\AsArrayObject;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Casts\AsCollection;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Casts\Attribute;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\InvalidCastException;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\JsonEncodingException;
use OtomaticAi\Vendors\Illuminate\Database\Eloquent\Relations\Relation;
use OtomaticAi\Vendors\Illuminate\Database\LazyLoadingViolationException;
...
trait HasAttributes
{
    ...
    protected function getRelationshipFromMethod($method)
    {
        ...
        return \OtomaticAi\Vendors\tap($relation->getResults(), function ($results) use($method) {
    ...
    {
        return \OtomaticAi\Vendors\tap($this->getArrayAttributeByKey($key), function (&$array) use($path, $value) {
    ...
    protected function getOriginalWithoutRewindingModel($key = null, $default = null)
    {
        ...
        return \OtomaticAi\Vendors\collect($this->original)->mapWithKeys(function ($value, $key) {
    ...
    {
        static::$getAttributeMutatorCache[$class] = \OtomaticAi\Vendors\collect($attributeMutatorMethods = static::getAttributeMarkedMutatorMethods($class))->mapWithKeys(function ($match) {
            return [\lcfirst(static::$snakeAttributes ? Str::snake($match) : $match) => \true];
        })->all();
        static::$mutatorCache[$class] = \OtomaticAi\Vendors\collect(static::getMutatorMethods($class))->merge($attributeMutatorMethods)->map(function ($match) {
    ...
    protected static function getAttributeMarkedMutatorMethods($class)
    {
        ...
        return \OtomaticAi\Vendors\collect((new ReflectionClass($instance))->getMethods())->filter(function ($method) use($instance) {

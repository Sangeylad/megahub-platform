...
use OtomaticAi\Vendors\Egulias\EmailValidator\EmailValidator;
use OtomaticAi\Vendors\Egulias\EmailValidator\Validation\DNSCheckValidation;
use OtomaticAi\Vendors\Egulias\EmailValidator\Validation\MultipleValidationWithAnd;
use OtomaticAi\Vendors\Egulias\EmailValidator\Validation\NoRFCWarningsValidation;
use OtomaticAi\Vendors\Egulias\EmailValidator\Validation\RFCValidation;
use OtomaticAi\Vendors\Egulias\EmailValidator\Validation\SpoofCheckValidation;
use Exception;
...
use OtomaticAi\Vendors\Illuminate\Validation\ValidationData;
use InvalidArgumentException;
use OtomaticAi\Vendors\Symfony\Component\HttpFoundation\File\File;
use OtomaticAi\Vendors\Symfony\Component\HttpFoundation\File\UploadedFile;
trait ValidatesAttributes
{
    ...
    public function validateEmail($attribute, $value, $parameters)
    {
        ...
        $validations = \OtomaticAi\Vendors\collect($parameters)->unique()->map(function ($validation) {
            if ($validation === 'rfc') {
    ...
    {
        if (\in_array($attribute, Arr::collapse($this->implicitAttributes)) && !\is_numeric($last = \OtomaticAi\Vendors\last(\explode('.', $attribute)))) {
            return $last;
        }
        return $attribute;
    }

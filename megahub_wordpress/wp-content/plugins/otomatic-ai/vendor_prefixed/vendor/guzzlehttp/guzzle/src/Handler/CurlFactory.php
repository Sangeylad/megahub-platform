...
namespace OtomaticAi\Vendors\GuzzleHttp\Handler;

use OtomaticAi\Vendors\GuzzleHttp\Exception\ConnectException;
use OtomaticAi\Vendors\GuzzleHttp\Exception\RequestException;
use OtomaticAi\Vendors\GuzzleHttp\Promise as P;
use OtomaticAi\Vendors\GuzzleHttp\Promise\FulfilledPromise;
use OtomaticAi\Vendors\GuzzleHttp\Promise\PromiseInterface;
use OtomaticAi\Vendors\GuzzleHttp\Psr7\LazyOpenStream;
use OtomaticAi\Vendors\GuzzleHttp\TransferStats;
use OtomaticAi\Vendors\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \OtomaticAi\Vendors\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \OtomaticAi\Vendors\GuzzleHttp\Psr7\Utils::streamFor($sink);

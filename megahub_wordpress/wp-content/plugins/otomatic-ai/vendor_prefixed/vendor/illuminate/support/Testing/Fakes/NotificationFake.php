...
use Closure;
use Exception;
use OtomaticAi\Vendors\Illuminate\Contracts\Notifications\Dispatcher as NotificationDispatcher;
use OtomaticAi\Vendors\Illuminate\Contracts\Notifications\Factory as NotificationFactory;
use OtomaticAi\Vendors\Illuminate\Contracts\Translation\HasLocalePreference;
use OtomaticAi\Vendors\Illuminate\Notifications\AnonymousNotifiable;
...
class NotificationFake implements NotificationDispatcher, NotificationFactory
{
    ...
    {
        $actualCount = \OtomaticAi\Vendors\collect($this->notifications)->flatten(1)->reduce(function ($count, $sent) use($notification) {
    ...
    public function sent($notifiable, $notification, $callback = null)
    {
        ...
        $notifications = \OtomaticAi\Vendors\collect($this->notificationsFor($notifiable, $notification));
    ...
    public function sendNow($notifiables, $notification, array $channels = null)
    {
        ...
        foreach ($notifiables as $notifiable) {
            ...
            $this->notifications[\get_class($notifiable)][$notifiable->getKey()][\get_class($notification)][] = ['notification' => $notification, 'channels' => $notifiableChannels, 'notifiable' => $notifiable, 'locale' => $notification->locale ?? $this->locale ?? \OtomaticAi\Vendors\value(function () use($notifiable) {

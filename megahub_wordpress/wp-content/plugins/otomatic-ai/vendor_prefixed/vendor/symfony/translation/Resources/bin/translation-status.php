...
foreach ($config['original_files'] as $originalFilePath) {
    $translationFilePaths = \OtomaticAi\Vendors\findTranslationFiles($originalFilePath, $config['locale_to_analyze']);
    $translationStatus = \OtomaticAi\Vendors\calculateTranslationStatus($originalFilePath, $translationFilePaths);
    ...
    \OtomaticAi\Vendors\printTranslationStatus($originalFilePath, $translationStatus, $config['verbose_output'], $config['include_completed_languages']);
}
...
function findTranslationFiles($originalFilePath, $localeToAnalyze)
{
    ...
    foreach ($translationFiles as $filePath) {
        $locale = \OtomaticAi\Vendors\extractLocaleFromFilePath($filePath);
...
{
    $translationStatus = [];
    $allTranslationKeys = \OtomaticAi\Vendors\extractTranslationKeys($originalFilePath);
    foreach ($translationFilePaths as $locale => $translationPath) {
        $translatedKeys = \OtomaticAi\Vendors\extractTranslationKeys($translationPath);
        $missingKeys = \array_diff_key($allTranslationKeys, $translatedKeys);
        $mismatches = \OtomaticAi\Vendors\findTransUnitMismatches($allTranslationKeys, $translatedKeys);
        ...
        $translationStatus[$locale]['is_completed'] = \OtomaticAi\Vendors\isTranslationCompleted($translationStatus[$locale]);
    }
...
{
    \OtomaticAi\Vendors\printTitle($originalFilePath);
    \OtomaticAi\Vendors\printTable($translationStatus, $verboseOutput, $includeCompletedLanguages);
...
function printTable($translations, $verboseOutput, bool $includeCompletedLanguages)
{
    ...
    foreach ($translations as $locale => $translation) {
        ...
        \OtomaticAi\Vendors\textColorNormal();

...
log "🚀 Déploiement MegaHub Frontend React 19 v3.0.0 - STAGING"
...
docker-compose -f docker-compose.staging.yml stop frontend 2>/dev/null || true
...
if ! docker-compose -f docker-compose.staging.yml ps frontend | grep -q "Up\|healthy"; then
...
INTERNAL_HEALTH=""
for i in {1..6}; do
    if INTERNAL_HEALTH=$(docker exec megahub-frontend-staging curl -f http://localhost/health 2>/dev/null); then
...
HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" http://staging.megahub.humari.fr/health 2>/dev/null || echo "000")
...
HTTPS_HEALTH=""
for i in {1..10}; do
    if HTTPS_HEALTH=$(curl -f -s --max-time 10 https://staging.megahub.humari.fr/health 2>/dev/null); then
...
RESPONSE_TIME=$(curl -w "%{time_total}" -o /dev/null -s --max-time 15 https://staging.megahub.humari.fr/ 2>/dev/null || echo "ERROR")
...
BUILD_INFO=$(curl -s --max-time 5 https://staging.megahub.humari.fr/build-info 2>/dev/null || echo "N/A")
...
success "🎉 Déploiement React 19 STAGING réussi!"
react19_log "Application staging disponible: https://staging.megahub.humari.fr"
log "📈 Monitoring staging: docker-compose -f docker-compose.staging.yml logs frontend"
log "🔍 Debugging: docker exec megahub-frontend-staging curl http://localhost/health"
log "⚡ Performance: curl -w '%{time_total}' https://staging.megahub.humari.fr/"
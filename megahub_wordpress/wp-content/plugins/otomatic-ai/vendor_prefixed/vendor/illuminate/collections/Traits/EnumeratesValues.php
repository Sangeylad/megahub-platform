...
use OtomaticAi\Vendors\Illuminate\Support\Enumerable;
use OtomaticAi\Vendors\Illuminate\Support\HigherOrderCollectionProxy;
use OtomaticAi\Vendors\Illuminate\Support\HigherOrderWhenProxy;
use JsonSerializable;
use OtomaticAi\Vendors\Symfony\Component\VarDumper\VarDumper;
use Traversable;
...
trait EnumeratesValues
{
    ...
    public function containsStrict($key, $value = null)
    {
        if (\func_num_args() === 2) {
            return $this->contains(function ($item) use($key, $value) {
                return \OtomaticAi\Vendors\data_get($item, $key) === $value;
            });
        }
    ...
    public function whereIn($key, $values, $strict = \false)
    {
        ...
        return $this->filter(function ($item) use($key, $values, $strict) {
            return \in_array(\OtomaticAi\Vendors\data_get($item, $key), $values, $strict);
        });
    }
    /**
     * Filter items by the given key value pair using strict comparison.
    ...
    public function whereNotBetween($key, $values)
    {
        return $this->filter(function ($item) use($key, $values) {
            return \OtomaticAi\Vendors\data_get($item, $key) < \reset($values) || \OtomaticAi\Vendors\data_get($item, $key) > \end($values);
        });
    }
    /**
     * Filter items by the given key value pair.
    ...
    public function reduceSpread(callable $callback, ...$initial)
    {
        ...
        foreach ($this as $key => $value) {
            ...
            if (!\is_array($result)) {
                throw new UnexpectedValueException(\sprintf("%s::reduceMany expects reducer to return an array, but got a '%s' instead.", \OtomaticAi\Vendors\class_basename(static::class), \gettype($result)));
            }
        }
        return $result;
    }
    ...
    protected function operatorForWhere($key, $operator = null, $value = null)
    {
        ...
        return function ($item) use($key, $operator, $value) {
            $retrieved = \OtomaticAi\Vendors\data_get($item, $key);
    ...
    protected function valueRetriever($value)
    {
        ...
        return function ($item) use($value) {
            return \OtomaticAi\Vendors\data_get($item, $value);
        };
    }
    /**
